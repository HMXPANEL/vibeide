...
import com.hmx.ide.utils.FlashType
import com.hmx.ide.utils.flashMessage
...
class RenameAction(context: Context, override val order: Int) :
  ...
  override suspend fun execAction(data: ActionData) {
    ...
    binding.name.editText!!.hint =
      context.getString(com.hmx.ide.resources.R.string.new_name)
    binding.name.editText!!.setText(file.name)
    builder.setTitle(com.hmx.ide.resources.R.string.rename_file)
    builder.setMessage(com.hmx.ide.resources.R.string.msg_rename_file)
    builder.setView(binding.root)
    builder.setNegativeButton(android.R.string.cancel, null)
    builder.setPositiveButton(com.hmx.ide.resources.R.string.rename_file) {
      dialogInterface,
      _ ->
      dialogInterface.dismiss()
      actionScope.launchAsyncWithProgress(
          configureFlashbar = { builder, cancelChecker ->
            builder.message(com.hmx.ide.resources.R.string.please_wait)
          },
          action = { _, _ ->
            ...
            withContext(Dispatchers.Main) {
              flashMessage(
                  if (renamed) com.hmx.ide.resources.R.string.renamed
                  else com.hmx.ide.resources.R.string.rename_failed,
