 ...
 * Original: app/src/main/cpp/pocket_spawn.c
 *
...
Java_com_hmx_webide_ai_terminal_NativeSpawn_spawn(JNIEnv *env, jobject self, jobjectArray java_argv,
    ...
    if (pid == 0) {
        ...
        // also stops commands spawned underneath it.
        setpgid(0, 0);
        close(in_pipe[1]);
...
Java_com_hmx_webide_ai_terminal_NativeSpawn_waitFor(JNIEnv *env, jobject self, jint pid, jboolean no_hang) {
    (void)env;
    (void)self;
    int status = 0;
...
JNIEXPORT jint JNICALL
Java_com_hmx_webide_ai_terminal_NativeSpawn_kill(JNIEnv *env, jobject self, jint pid, jint signal) {
    (void)env;
    (void)self;
